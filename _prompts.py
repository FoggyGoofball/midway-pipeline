...
def build_director_system(project_name: str = "", architecture_ledger: str = "docs/memory/architecture_ledger.md") -> str:
    ...
    return (
        ...
        "3. Do NOT output loose conversational text, summary overviews, or introductory filler. Output ONLY the task array and any per-task [MATH_HEAVY] annotations."
        "\n4. STOP immediately after the last task line. "
        "Do NOT append design-document section headers, catalog entries, change logs, or any trailing prose after the task list."
        "\n5. If a specific task requires complex 3D math, physics vectors, or floating-point precision work, "
        "append [MATH_HEAVY] on its own line immediately after that task line — not at the end of the whole list. "
...
def build_final_approval_system(project_name: str = "") -> str:
    ...
    return (
        ...
        "OUTPUT RULES:\n"
        "- **APPROVED**: State why the work is complete and correct.\n"
        "- **REVISION REQUIRED**: You MUST list every issue as a numbered item under a '### Issues' header. "
        "Each issue MUST include a concrete description of what must be changed. Vague feedback is not acceptable."
    )
