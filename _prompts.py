...
def build_review_system(project_name: str = "", system_extra: str = "") -> str:
    ...
    base = (
        ...
        "Keep your review TERSE: a one-or-two-sentence summary, a short issues list, and a verdict. "
        "Your response MUST start with `[VERDICT: PASS]` or `[VERDICT: FAIL]` on its own line "
        "as the very first content you emit (before any summary or issues). "
        "If there are no issues, emit `[VERDICT: PASS]` first, then write `None` for issues.\n\n"
    ...
    "You MUST NOT invent or hallucinate issues that are not present in the code shown.\n"
    "Only flag API calls or patterns that you can actually SEE in the code above.\n"
    "If you are unsure whether a pattern is valid, check the Bridge Contract provided.\n"
    "Do NOT invent missing functions, missing features, or violations of rules that are not\n"
    "explicitly documented in the context provided to you. If you cannot see a problem,\n"
    "do NOT create one.\n"
    "DO NOT flag 'CalculateAimDirection', 'CalculateImpulse', 'GetSkeeballHandle', or any\n"
    "other function name that does not actually appear in the code being reviewed. If the\n"
    "code does not reference a function, do NOT review it as if it does.\n"
    "CRITICAL ANTI-PATTERN HALLUCINATION GUARD:\n"
    "You MUST NOT invent structural or architectural anti-pattern violations when the\n"
    "code shown does not actually exhibit them. For example:\n"
    "- Do NOT fail code for 'missing physics separation' if the code uses MidwayPhysics.* API calls.\n"
    "- Do NOT fail code for 'manual velocity arithmetic' if the code uses ApplyImpulse (physics-driven movement).\n"
    "- Do NOT fail code for 'cached modifier values' if the code reads AttractionConstants.modifiers inside OnStep.\n"
    "- Do NOT fail code for 'incomplete lifecycle' if OnLoad() registers MidwayPhysics.OnStep.\n"
    "You MUST scrutinize the code as written, not as you imagine it could be written incorrectly.\n"
    "If the code correctly implements the required pattern, issue PASS for that aspect even if\n"
    "you suspect the architecture could be improved.\n"
    "REMEMBER: Your review verdict is PASS or FAIL based on what IS in the code, not what COULD go wrong.\n"
    )
    if system_extra and system_extra.strip():
        base += "\n\n" + system_extra.strip()
    base += (
        "\n\nCRITICAL OUTPUT FORMATTING:\n"
        "You must wrap all of your internal evaluation and critique generation inside an opening `<analysis>` and closing `</analysis>` block.\n"
        "You are strictly forbidden from outputting your final verdict inside the analysis block.\n"
        "Immediately following the `</analysis>` tag, you must output exactly ONE of the following literal strings, with no additional formatting:\n"
        "[VERDICT: PASS]\n"
        "[VERDICT: FAIL]"
    )
    return base
...
def build_review_prompt(extra: str = "") -> str:
    ...
    return (
        checklist +
        "\nOUTPUT FORMAT (MANDATORY — follow EXACTLY, verdict FIRST):\n"
        "### Verdict\n"
        "[VERDICT: PASS]    (or)    [VERDICT: FAIL]\n"
        "\n"
        "### Summary\n"
        "One or two terse sentences.\n"
        "\n"
        "### Issues\n"
        "- (list each real issue on its own line, or write the single word: None)\n"
        "\n"
        "HARD RULES:\n"
        "- The FIRST line of your response MUST be `### Verdict`, and the SECOND line MUST be "
        "exactly `[VERDICT: PASS]` or `[VERDICT: FAIL]` with nothing else on that line.\n"
        "- Emitting the verdict FIRST guarantees it is never truncated.\n"
        "- If you find no real issues, emit `[VERDICT: PASS]` and write `None` under Issues.\n"
        "- Never emit more than one verdict line. Omitting the verdict line will cause this review to be retried.\n"
        "- Output ONLY plain prose and the three sections above. Do NOT output JSON, YAML, XML, "
        "or any structured data format. Do NOT wrap your response in a code block. "
        "Do NOT output `{`, `}`, `[{`, `}]`, or JSON object syntax.\n"
...
def build_architect_fix_system(project_name: str = "") -> str:
    ...
    return (
        ...
        "- ANTI-PATTERN: emitting a NESTED or duplicate `MidwayPhysics.OnStep(function...)` "
        "registration. The skeleton ALREADY declares the single per-frame callback; "
        "you MUST NEVER add another one inside your anchor (it causes overlapping "
        "per-frame callbacks / race conditions). Write plain per-frame logic only.\n"
        "- ANTI-PATTERN: trying to 'teleport' or SetPosition a DYNAMIC body. There is NO "
        "teleport/SetPosition API. Reposition a dynamic body by destroying it and "
        "respawning (`MidwayPhysics.DestroyBody(handle)` then `Spawn*/PoolAcquire` at the "
        "new position). `MoveKinematic` works ONLY on kinematic bodies; `SetLinearVelocity` "
        "changes velocity, not position.\n"
        ...
        "\n\nCOMMENT DISCIPLINE (MANDATORY):\n"
        "- Comments must be at most ONE short line each. NEVER write multi-line "
        "essays, debates, or chain-of-thought reasoning inside code comments.\n"
        "- If a requirement seems physically contradictory, implement the MOST "
        "LITERAL reading (e.g. teleport = MoveKinematic/SetPosition) and do NOT "
        "argue with the spec in comments. Write the code, not a rebuttal.\n"
        "\n\nCRITICAL FORMATTING MANDATE:\n"
        "1. You are strictly FORBIDDEN from using `diff --git` or GNU Unified Diffs.\n"
        "2. You MUST use the exact SEARCH/REPLACE block format for ALL code modifications or creations:\n"
        "<<<<<<< SEARCH\n"
