        ...
        for lf in _lua_root.rglob("*.lua"):
            ...
            try:
                ...
                if lua_proc.returncode != 0:
                    ...
                    # A one-token `end` imbalance is the classic "so close" failure:
                    # every task applies but luac rejects the whole file, and the
                    # per-task LLM fix loop cannot repair a structural token it can
                    # never see. Fix it here before injecting any error. The fix is
                    # only committed when re-running luac proves it clean, so a
                    # deeper structural wreck is left for the fix loop / surgery.
                    _bal_fixed_syntax = False
                    if _owning_tid:
                        try:
                            ...
                            if _bal_actions and _bal_out != _bal_src:
                                ...
                                try:
                                    ...
                                    try:
                                        _bal_os.unlink(_bal_tmp)
                                    except Exception:
                                        pass
                                if _bal_luac.returncode == 0:
                                    lf.write_text(_bal_out, encoding="utf-8")
                                    _bal_tf = ""
                                    if _owning_tid in ctx.task_map:
                                        _bal_tf = str(getattr(ctx.task_map[_owning_tid], "target_file", "") or "")
                                    if _bal_tf:
                                        _bal_real = ctx.project_root / _bal_tf
                                        try:
                                            _bal_real.parent.mkdir(parents=True, exist_ok=True)
                                            _bal_real.write_text(_bal_out, encoding="utf-8")
                                        except Exception:
                                            pass
                                    if _owning_tid.startswith("task_") and _owning_tid in ctx.all_results_dict:
                                        ctx.all_results_dict[_owning_tid] = _bal_out
