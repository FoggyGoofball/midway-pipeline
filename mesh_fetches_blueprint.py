...
def _run_blueprint_phase(ctx: PipelineContext, blueprint_path, gdd_snippet: str, state_snippet: str) -> PipelineContext:
    ...
    _blueprint_system = (
        ...
        "9. For NEW_ATTRACTION requests, the pipeline ALREADY generates the file skeleton: "
        "OnLoadStatic, OnLoad, OnUnload and the MidwayPhysics.OnStep registration are pre-written, "
        "and all module-level state variables are pre-declared. The FIRST task MUST fill the "
        "module-level state / constants / helper-function anchor ONLY — it must NOT re-define "
        "OnLoadStatic/OnLoad/OnUnload/OnStep (those hooks already exist). Use ONLY the real hook names: "
        + (", ".join(_bp_lifecycle_hooks) if _bp_lifecycle_hooks else "OnLoadStatic, OnLoad, OnUnload, OnStep")
        + ". "
        "Do NOT invent wrapper functions like OnLoadAttraction(). OnStep is registered by the "
        "skeleton via MidwayPhysics.OnStep(function(dt) ... end) inside OnLoad() — "
        "do NOT re-create it in any task.\n"
        ...
        + "\n"
        + (
            "15. ANCHOR-PATCH RULE (CRITICAL — the orchestrator owns the scaffold, NOT task 1):\n"
            "    - The orchestrator ALREADY writes the complete .lua file: all lifecycle entry "
            "points (OnLoadStatic, OnLoad, OnStep registration, OnUnload), module-level state "
            "variables, and TODO anchor markers are pre-generated before any task runs.\n"
            "    - EVERY task (including Task 1) outputs ONLY a SEARCH/REPLACE block that targets "
            "its own assigned TODO anchor. NO task may output a full file or redefine the lifecycle hooks.\n"
            "    - Task 1 fills the module-level state / constants / helper-function anchor.\n"
            "    - Tasks 2..N each fill exactly one other anchor (geometry, pools, input, scoring, cleanup).\n"
            "    - Every task MUST end its REPLACE by re-emitting the same anchor marker line it "
            "searched for, so the next task can still find it.\n"
        )
