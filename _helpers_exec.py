                ...
                if _live_content.strip():
                    ...
                    if _anchor_marker:
                        ...
                        _stage_block = (
                            ...
                            f"- There is NO teleport/SetPosition API for dynamic bodies. To move a "
                            f"dynamic body mid-flight: `MidwayPhysics.DestroyBody(handle)` then "
                            f"`PoolAcquire`/`SpawnDynamic*` at the new position. `MoveKinematic` "
                            f"moves ONLY kinematic bodies; `SetLinearVelocity` sets velocity "
                            f"(it does NOT reposition).\n"
