...
- [x] **Task 3 (Directory Scaffold):** `global_cache/` exists. Note (verified 2026-09-02): `profiles/`, `global_cache/api_docs/`, and `global_cache/cache_index.json` are absent on disk (empty dirs aren't git-tracked; `wizard.py --init <name>` recreates `profiles/<name>/manifest.json` on demand).
