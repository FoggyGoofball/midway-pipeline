    ...
    while ctx.review_cycle < _REVIEW_MAX_ITERATIONS:
        ...
        if ctx.review_verdict == "FAIL" and ctx.review_cycle < _REVIEW_MAX_ITERATIONS:
            ...
            if _mono_target:
                ...
                # -- Whole-file skeleton echo rejection ------------------------
                # The fix coder sometimes ignores SEARCH/REPLACE and re-emits the
                # entire skeleton (with the _build_skeleton.py header + TODO stubs)
                # instead of a surgical patch. Such an echo never matches on-disk
                # content and the full-file-diff fallback would glue the skeleton
                # back in. Blank it so the existing Post-Fix Validation reverts
                # the task to its pre-fix snapshot instead of applying garbage.
                # First check is language-agnostic (PatchContract); second is a
                # Midway-specific fast-path signal for the skeleton header.
                try:
                    from patch_contract import PatchContract, Rule as _PC_Rule
                    _pc_waffle = PatchContract.for_lua().reject_waffle(agent_fix_output or "")
                except Exception:
                    _pc_waffle = None
                if _pc_waffle is not None:
                    print(f"  [Anchor Fix] ⚠ {tid}: contract reject ({_pc_waffle.value}) — "
                          f"blanking output (Post-Fix will revert to pre-fix snapshot).")
                    agent_fix_output = ""
                elif "Auto-generated by _build_skeleton" in (agent_fix_output or ""):
                    print(f"  [Anchor Fix] ⚠ {tid}: whole-file skeleton echo detected — "
                          f"blanking output (Post-Fix will revert to pre-fix snapshot).")
                    agent_fix_output = ""
